...
class CustomerLogin extends Component{
    constructor(){
        super();
        this.state = {
            loginResult : <div>Please login</div>
        }
    ...
    async login(e) {
        ...
        const url_login = '/api/customer/login';
        const header = {
            headers: { 
        ...
        const response_login = await axios.post(url_login, params, header)

        if(response_login.data.result === true){
            if(response_login.data.password_reset){
                this.setState({
                    loginResult : <div><Redirect to="/customer/ResetPassword"/></div>
                });
            }
            else{
                this.setState({
                    loginResult : <div><Redirect to="/"/></div>
                })
            }
        }
        else{
            this.setState({
                loginResult : <div style={{color:'red'}}>{response_login.data.message}</div>
            })
        }
    }
                    ...
                    <Form onSubmit={this.login} style={{padding:15}}>
                        ...
                        <div style={{paddingTop:20}}>
                            <Row style={{paddingTop:10}}>
                                <Col><NavLink to={`/customer/Register`}><Button size="lg" block>Register Now!</Button></NavLink></Col>
                            </Row>
                            <Row style={{paddingTop:10}}>
                                <Col><NavLink to={`/customer/FindPassword`}><Button variant="danger" size="lg" block>Forgot Password?</Button></NavLink></Col>
                            </Row>
                        </div>
                    </Form>
                </div>
