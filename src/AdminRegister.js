...
class Register extends Component{
    ...
    register(event) {
        ...
        if(this.state.passwordCheck && this.state.idCheck && this.state.emailCheck){            
            ...
            params.append('id', event.target.id.value);
            ...
            .then((response) => {
                if(response){
                    this.setState({
                        registerResult : <div><Redirect to="/admin/login"/></div>
                    });
                }
                else{
                    this.setState({
    ...
    idCheck(e){
        ...
        axios.post('/api/admin/register/idcheck', params,{
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
        }).then((response) => {
                ...
                <div>
                    <Form onSubmit={this.register} style={{padding:15}}>
                        <Form.Group as={Row} controlId="id">
                            <Col>
                                <Form.Control type="text" name="id" placeholder="ID" autoComplete='off' onChange={this.idCheck}/>
                            </Col>
                        </Form.Group>
                        ...
                        <Form.Group as={Row} controlId="email">
                            <Col>
                                <InputGroup>
                                    <Form.Control type="email" name="email"autoComplete='off' onChange={this.emailCheck} placeholder="Email"/>
                                </InputGroup>
                                <Form.Text className="text-muted" style={{textAlign:'left'}}>
